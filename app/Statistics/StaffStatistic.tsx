...
interface SaleData {
  staff_name: string;
  total_accept: number;
  total: number; // Đổi từ `total_price` thành `total`
}
...
const CakeStatistic: React.FC = () => {
  ...
  const barChartData = {
    labels: salesData.map((item) => item.staff_name.split(" ")[0]),
    datasets: [
      {
        data: salesData.map((item) => item.total.toFixed(2)), // Sử dụng trường `total`
        colors: colors.map(
          (color) =>
            (opacity = 1) =>
              `${color}`
            ...
            <View style={styles.barChartContainer}>
              <Animated.View style={animatedStyle}>
                <BarChart
                  ...
                  yAxisLabel="$"
                  width={
                    (Dimensions.get("window").width / 4) * salesData.length
                  }
                  height={250}
                  chartConfig={{
